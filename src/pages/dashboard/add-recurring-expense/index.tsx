...
import { IRecurringExpenseModel, RECURRING_EXPENSES_FIELDS_TYPE } from '../../../types';
...
const AddRecurringExpensePage = (props: IAddRecurringExpenseProps) => {
  ...
  const BREADCRUMBS: BreadcrumbProps[] = [
    ...
    { href: URLS.getPagePath('recurring-expense', { clientId }), icon: 'document', text: URLS.getPagePathName('recurring-expense') },
    // { text: URLS.getPagePathName('add-recurring-expense') }
  ];
  if (props.update) {
   ...
   */
   if (props.recurringExpense) {
    initialValues = Object.assign(
      ...
      pick(props.recurringExpense.recurringExpense, Object.keys(helpers.initialValues))
    );
  } else {
  ...
  return (
    <div className='add-recurring-expense'>
      <PageHeading
        title={props.update ? 'Update Recurring Expense' : 'Add Recurring Expense'}
      ...
      <div className='add-recurring-expense__container'>
        <Formik
            ...
            onSubmit={async (values, { resetForm, setSubmitting }) => {
              ...
              try {
                if (props.update) {
                  await api.recurringExpense.updateRecurringtExpense(get(props.recurringExpense, 'id'), values);
                } else {
                  await api.recurringExpense.createRecurringtExpense(values, { clientId });
                }
            ...
            {({
              ...
              const getNumericInput = (key: RECURRING_EXPENSES_FIELDS_TYPE, inputOptions = {}) => (
                <FormGroup
              ...
              const getTextAreaFormGroup = (key: RECURRING_EXPENSES_FIELDS_TYPE) => (
                <FormGroup
                  intent={helpers.getFormIntent(errors[key])}
                  label={get(FIELDS, key, { name: '' }).name}
